...
import { FiMail, FiUsers, FiMenu, FiUser, FiMessageCircle, FiFile, FiBox, FiClipboard, FiMic } from 'react-icons/fi';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['business-owner', 'auction-manager', 'inventory-specialist', 'customer-service-representative'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<AuctioneerInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/auctioneers` : null),
    () => getAuctioneers({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Auctioneer',
      path: isTenantUser ? `/auctioneers/view/${data?.[0]?.id}` : '/auctioneers',
      entity: 'auctioneer',
      service: AccessServiceEnum.PROJECT,
      icon: FiMic,
    },
    { name: 'Auction', path: '/auctions', entity: 'auction', service: AccessServiceEnum.PROJECT, icon: FiClipboard },
    { name: 'Item', path: '/items', entity: 'item', service: AccessServiceEnum.PROJECT, icon: FiBox },
