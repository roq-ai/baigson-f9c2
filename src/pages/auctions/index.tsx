...
function AuctionListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<AuctionInterface[]>(
    ...
    () =>
      getAuctions({
        relations: ['auctioneer', 'user', 'item.count'],
      }),
  );
          ...
          <TableContainer>
            <Table variant="simple">
              <Thead>
                <Tr>
                  <Th>name</Th>
                  {hasAccess('auctioneer', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && <Th>auctioneer</Th>}
                  {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && <Th>user</Th>}
                  {hasAccess('item', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && <Th>item</Th>}
                  <Th>Actions</Th>
                </Tr>
              </Thead>
              <Tbody>
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('auctioneer', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/auctioneers/view/${record.auctioneer?.id}`}>
                          {record.auctioneer?.name}
                        </Link>
                      </Td>
                    )}
